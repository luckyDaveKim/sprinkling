...
@DataJpaTest
public class SprinkledMoneyRepositoryTests {
  ...
  @Test
  @DisplayName("돈 뿌린 정보를 저장하고 조회 가능")
  public void shouldSaveAndFindSprinkledMoney() {
    /* Given */
    ...
    /* When */
    testEntityManager.persist(sprinkledMoney);

    /* Then */
    IdDto idDto = sprinkledMoneyRepository.findFirstIdByTokenAndRoomIdOrderByCreatedAtDesc(token, roomId).get();
    Assertions.assertEquals(sprinkledMoney, sprinkledMoneyRepository.findById(idDto.getId()).get());
  }
